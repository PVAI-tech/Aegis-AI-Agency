...
There is no test suite, no CI pipeline, and no test framework installed (consistent with the no-build-tooling constraint — see [`ADR.md`](ADR.md)). Everything to date has been verified through:
...
3. **Lighthouse CI**: no baseline has been captured against the live deployment yet (see [`Roadmap.md`](Roadmap.md)). Once captured, wiring it into a CI step (even a manually-triggered one) would catch performance/accessibility regressions.
