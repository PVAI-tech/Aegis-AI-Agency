...
| `pricing.html` | Productised AI section rewritten: new intro copy (UK spelling "Productised AI", explains setup-vs-monthly plainly), card markup replaced with a JS-rendered container, new usage-policy section, new minimum-term/billing section, comparison table + accordion, add-ons section, exclusions section, trust/AI-safety section, client-responsibilities section, and a 16-question pricing FAQ. Custom Development section (Motion 1) left untouched — its numbers weren't in scope for this pass. |
...
## Promotion to live (post-approval)

Once the draft was reviewed and approved, it was promoted from the throwaway `pricing-mockup.html` draft to a real, indexed page:

- **New `pricing.html`** — full site chrome added (loader, sticky CTA, nav with mobile menu, footer with the Legal column, WhatsApp button, Jarvis widget, shared modal, all standard script tags) matching every other real page, replacing the mockup's stripped-down nav-only shell. `noindex` removed; proper `<title>`/description/canonical/OG tags added.
- **Homepage cross-link** — `index.html`'s existing Custom Development pricing section (left otherwise unchanged) now ends with a link to `pricing.html` for visitors wanting the subscription option instead of a one-off project.
- **Site-wide nav update** — the "Pricing" link in the nav, mobile menu, and footer "Company" column was repointed from `index.html#pricing` to `pricing.html` across all 15 pages (previously it scrolled to the homepage's Custom-Development-only section; now it goes to the complete pricing page covering both motions).
- **`sitemap.xml`** — `pricing.html` added at priority 0.9, next to the homepage and enquiry page.
- **`pricing-mockup.html` deleted** — fully superseded by `pricing.html`; nothing left behind referencing the old filename in any HTML/XML file (docs were updated to reference the file by its new name where they historically described it).
- **Also fixed while touching this section:** the Growth System card's leftover `pfloat` floating-animation (a pre-existing artifact missed during the earlier site-wide rebrand pass that removed similar "unnecessary animation" clichés) was removed from `assets/site.css`.

This page is now live, indexed, and linked from every page on the site — not a draft.
