...
The moment enquiry volume makes "read every notification email manually" unworkable, or the Productized AI line ([`BusinessModel.md`](BusinessModel.md)) needs to track client subscriptions/usage, a real data store becomes necessary. See [`UsageMetering.md`](UsageMetering.md) for the full architecture the usage-tracking case specifically requires. Recommended options at that point, roughly in order of effort:
