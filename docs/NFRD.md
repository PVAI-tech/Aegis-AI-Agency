...
## Security
See [`Security.md`](Security.md) for full detail. Summary: security headers + CSP live via `vercel.json`, rate limiting on the enquiry endpoint, all user input HTML-escaped before being placed in emails. Known tradeoff: CSP allows `'unsafe-inline'` for scripts/styles (see [`Security.md`](Security.md) for why).
...
- No automated accessibility audit (axe-core/Lighthouse) has been run yet against the live site — see [`Roadmap.md`](Roadmap.md).
...
- Rate limiting is in-memory and per-serverless-instance (documented limitation in `api/enquiry.js` and [`Security.md`](Security.md)) — sufficient for current traffic, not a hard guarantee at scale.

## Browser/device support
See [`TRD.md`](TRD.md). No legacy-browser polyfills. Mobile experience has been explicitly tested and fixed for real (not assumed) issues — see the WhatsApp/hero-overlap fix in git history as an example of testing rather than assuming.
